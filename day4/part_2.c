...
void part_2() {
  ...
  int current_amount_of_rolls = initial_amount_of_rolls;
  int prev_amount_of_rolls = 0;
  while (current_amount_of_rolls != prev_amount_of_rolls) {
    prev_amount_of_rolls = current_amount_of_rolls;
    for (int y = 0; y < width; y += 1) {
      for (int x = 0; x < width; x += 1) {
        ...
        if (adjacent_rolls < 4) {
          floor_plan.items[pos_to_index((Coord){.x = x, .y = y}, width)] =
              Empty;
          current_amount_of_rolls -= 1;
        }
      }
    }
  }
  ...
  printf("part 2 answer: %d\n",
         initial_amount_of_rolls - current_amount_of_rolls);
}
